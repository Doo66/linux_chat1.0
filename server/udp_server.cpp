...
ssize_t udp_server::recv_msg()
{
	...
	if( -1 == _size){
		printf("recv data from cli failed!");
	}else{
...
//		printf("this is data fall into data_pool%s\n",_msg.c_str());      //test
...
//	printf("this is a test for sendto!!!!!!");
//	std::cout<<_msg<<std::endl;
//	printf("%d\n",_size);
	if( _size == -1 ){
		printf("send data to cli failed!");
	}else{
		printf("send data to cli success!");
	}
	return _size;
}
